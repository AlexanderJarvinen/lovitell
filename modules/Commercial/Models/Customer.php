        ...
        try {
            ...
            if (count($r)) {
                if (empty($r[0]->error)) {
                    ...
                    $this->p_apartment=$r[0]->p_apartment;
                    $this->phone=$r[0]->phone;
                    $this->email=$r[0]->e_mail;
                    $this->ac_id=$r[0]->ac_id;
                    $this->type=$r[0]->type;
                    $inn_kpp = explode('/', $r[0]->n4);
                    $account = explode('/', $r[0]->n3);
                    $this->recv = new Requisites();
                    $this->recv->init($r[0]->n1?Helper::cyr($r[0]->n1):'', $r[0]->n2?Helper::cyr($r[0]->n2):'', $account[0], $inn_kpp[0], isset($inn_kpp[1])?$inn_kpp[1]:'', $r[0]->n5, isset($account[1])?$account[1]:'');
                    $this->inited = true;
                } else {
