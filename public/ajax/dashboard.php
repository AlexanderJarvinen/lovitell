...
<?
    $ttt=1;
    while (chart('chart'.$ttt)){
	$out=chart('chart'.$ttt);
	?>
	<li>
    	    <a href="#" onClick=Wopen('<?='chart'.$ttt?>')>
    		    <?
		    if ($_COOKIE['chart'.$ttt.'close'] == 1) echo "<i id=\"ichart".$ttt."\" class=\"ace-icon fa fa-check-square-o\"> ";
		    else echo "<i id=\"ichart".$ttt."\" class=\"ace-icon fa fa-square-o\"> ";
...
?>
    ...
    <div class="col-xs-4 widget-container-col" id="col<?=$ii;?>" style="min-height: 292px">
	<? 
	    if (!$_COOKIE['col0'] and !$_COOKIE['col1'] and !$_COOKIE['col2']){
		if ($ii==0 ) $ai=array('chart1','chart5','chart8','chart4','chart13','chart11');
		else if ($ii==1 ) $ai=array('chart2','chart6','chart9', 'chart15', 'chart14');
		else if ($ii==2 ) $ai=array('chart3','chart7','chart10','chart16','chart12');
	    } else $ai=explode(' ',$_COOKIE[col.$ii]);
