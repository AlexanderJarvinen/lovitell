...
import * as JobConstants from '../constants/JobConstants.js';
...
export default class JobsList2 extends Component {
    static propTypes = {
    ...
    getJobs = () => {
        let {types, states, dates, onlyMy, address_id, routes} = this.props;
        dates = {
            start: dates.start.format('DD.MM.YYYY'),
            end: dates.end.format('DD.MM.YYYY')
        };
        $.get({
            url: '/inventory/ajax/jobs2?filters='+JSON.stringify({
                types,
                states,
                dates,
                onlyMy,
