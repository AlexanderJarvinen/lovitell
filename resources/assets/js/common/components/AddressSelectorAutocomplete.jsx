...
import RegionAutocomplete from './RegionAutocomplete.jsx';
import StreetAutocomplete from './StreetAutocomplete.jsx';
...
var Inbuild = React.createClass({
    ...
    render: function() {
        ...
        var entrance,
            floor,
            appartment = null;
        if (this.state.accuracy>1 && !this.props.skip_tech) {
            entrance = ( <div className={this.props.className}>
                <label htmlFor="entrance">Подъезд:</label>
                   {entr_field}
            </div>);
        }
        if (this.state.accuracy>2 && !this.props.skip_tech) {
...
export default class AddressSelectorAutocomplete extends Component {
    propTypes: {
        ...
        onBuildChange: React.PropTypes.func
    };
    constructor(props) {
        super(props);
        let accuracy = 4;
    ...
    render = () => {
        ...
        return (
            <div className="address_selector row">
